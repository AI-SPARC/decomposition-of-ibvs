...
class DensoVP6242Simulation():
    ...
    def fkine(self, recalculate=False, all_transforms=False):
        if recalculate:
            self.getJointsPos()
            self.T_0_1 = self.dh(self.q[0], 0.125, 0, -np.pi/2)
            self.T_0_2 = self.T_0_1 @ self.dh(self.q[1] - np.pi/2, 0, 0.210, 0)
            self.T_0_3 = self.T_0_2 @ self.dh(self.q[2] + np.pi, 0, -0.075, np.pi/2)
            self.T_0_4 = self.T_0_3 @ self.dh(self.q[3], 0.210, 0, -np.pi/2)
            self.T_0_5 = self.T_0_4 @ self.dh(self.q[4], 0.1157, 0, np.pi/2)
            self.T_0_6 = self.T_0_5 @ self.dh(self.q[5], 0.070, 0, 0)
    ...
    def computePose(self, recalculate_fkine=False):
        ...
        cameraPose = np.array(self.sim.getObjectPose(self.joints[-1] + self.sim.handleflag_wxyzquat, -1))
        ...
        pose[3], pose[4], pose[5] = utils.quat2taitbryan(cameraPose[3:7])
        print(cameraPose[3:7])

        return pose
    
    def computePose3(self, recalculate_fkine=False):
        return np.array(self.sim.getObjectPose(self.joints[-1] + self.sim.handleflag_wxyzquat, -1))
    
    def computePose2(self, recalculate_fkine=False):
        M = self.fkine(recalculate=recalculate_fkine)

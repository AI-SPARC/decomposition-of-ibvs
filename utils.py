...
def quat2euler(h):
    ...
    pitch = -np.pi/2 + 2*np.arctan2(np.sqrt(1 + 2*(h[0]*h[2] - h[1]*h[3])), np.sqrt(1 - 2*(h[0]*h[2] - h[1]*h[3])))
    yaw = np.arctan2(2*(h[0]*h[3] + h[1]*h[2]), 1 - 2*(h[2]**2 + h[3]**2))

    return (roll, pitch, yaw)

def quat2taitbryan(h):
    roll = np.arctan2(h[2]*h[3] + h[0]*h[1], 0.5 - h[1]**2 - h[2]**2)
    pitch = np.arcsin(2*(h[0]*h[2] - h[3]*h[1]))
    yaw = np.arctan2((h[0]*h[3] + h[1]*h[2]), 0.5 - h[2]**2 - h[3]**2)
